use project;

INSERT INTO Product  (ProductID, ProductName, Price, Quantity, Status)  VALUES
(1, 'BMW X5', 65000.00, 5, 'Available for purchase'),
(2, 'Mercedes C-Class', 45000, 3, 'Available for leasing'),
(3, 'Audi A4', 40000, 0, 'Available for purchase'),
(4, 'Tesla Model S', 80000, 1, 'Available for leasing'),
(5, 'Ford Focus', 20000.00, 10, 'Available for purchase'),
(6, 'Toyota Corolla', 18000, 8, 'Available for leasing'),
(7, 'Honda Civic', 22000, 0, 'Available for purchase'),
(8, 'Chevrolet Malibu', 25000, 4, 'Available for leasing'),
(9, 'Nissan Altima', 23000, 7, 'Available for purchase'),
(10, 'Hyundai Sonata', 24000, 9, 'Available for leasing'),
(11, 'Brake Pads Set', 120, 25, 'Available for purchase'),
(12, 'Engine Oil 5W-30 - 5L', 45, 50, 'Available for purchase'),
(13, 'Air Filter', 30, 40, 'Available for purchase'),
(14, 'Fuel Pump', 180, 15, 'Available for purchase'),
(15, 'Spark Plug Set (4 pcs)', 60, 30, 'Available for purchase'),
(16, 'Battery 12V 60Ah', 110, 20, 'Available for purchase'),
(17, 'Front Bumper - Toyota Corolla', 250, 5, 'Available for purchase'),
(18, 'Headlight - Honda Civic (Left)', 180, 8, 'Available for purchase'),
(19, 'Rearview Mirror - BMW X5 (Right)', 90, 12, 'Available for purchase'),
(20, 'Alternator - Ford Focus', 300, 7, 'Available for purchase'),
(21, 'Radiator - Nissan Altima', 270, 6, 'Available for purchase'),
(22, 'Clutch Kit - Audi A4', 400, 4, 'Available for purchase'),
(23, 'Transmission Fluid 1L', 35, 60, 'Available for purchase'),
(24, 'Wiper Blade Set', 25, 50, 'Available for purchase'),
(25, 'Oil Filter', 20, 45, 'Available for purchase'),
(26, 'Cabin Air Filter', 22, 35, 'Available for purchase'),
(27, 'Shock Absorber - Front Left', 140, 10, 'Available for purchase'),
(28, 'Serpentine Belt', 55, 18, 'Available for purchase'),
(29, 'Timing Chain Kit', 480, 3, 'Available for purchase'),
(30, 'Wheel Bearing Assembly', 130, 11, 'Available for purchase'),
(31, 'Brake Disc (Front)', 85, 16, 'Available for purchase'),
(32, 'Throttle Body', 310, 5, 'Available for purchase'),
(33, 'Power Steering Pump', 265, 6, 'Available for purchase'),
(34, 'Muffler Exhaust', 195, 9, 'Available for purchase'),
(35, 'Tail Light - Mercedes C-Class (Right)', 150, 7, 'Available for purchase'),
(36, 'Oxygen Sensor', 70, 20, 'Available for purchase'),
(37, 'Front Grille - Hyundai Sonata', 210, 5, 'Available for purchase'),
(38, 'Fuse Box Assembly', 90, 12, 'Available for purchase'),
(39, 'Control Arm - Rear Right', 165, 8, 'Available for purchase'),
(40, 'Starter Motor', 280, 4, 'Available for purchase'),
(41, 'Kia Sportage', 27000, 4, 'Available for leasing'),
(42, 'Volkswagen Tiguan', 32000, 5, 'Available for leasing'),
(43, 'Mazda CX-5', 29000, 3, 'Available for leasing'),
(44, 'Subaru Forester', 31000, 2, 'Available for leasing'),
(45, 'Toyota RAV4', 30000, 6, 'Available for leasing'),
(46, 'Hyundai Tucson', 28000, 5, 'Available for leasing'),
(47, 'Ford Escape', 29000, 3, 'Available for leasing'),
(48, 'Chevrolet Equinox', 27500, 4, 'Available for leasing'),
(49, 'Jeep Compass', 26000, 2, 'Available for leasing'),
(50, 'Nissan Rogue', 29500, 6, 'Available for leasing'),
(51, 'Peugeot 3008', 33000, 2, 'Available for leasing'),
(52, 'Renault Kadjar', 31000, 3, 'Available for leasing'),
(53, 'BMW X3', 55000, 1, 'Available for leasing'),
(54, 'Audi Q5', 60000, 2, 'Available for leasing'),
(55, 'Mercedes GLA', 58000, 1, 'Available for leasing'),
(56, 'Skoda Kodiaq', 35000, 4, 'Available for leasing'),
(57, 'Citroen C5 Aircross', 34000, 3, 'Available for leasing'),
(58, 'Dacia Duster', 22000, 7, 'Available for leasing'),
(59, 'Volvo XC40', 50000, 2, 'Available for leasing'),
(60, 'Mini Countryman', 37000, 3, 'Available for leasing'),
(61, 'Ford Mustang', 55000, 3, 'Available for purchase'),
(62, 'Chevrolet Camaro', 52000, 2, 'Available for purchase'),
(63, 'Toyota Camry', 27000, 6, 'Available for purchase'),
(64, 'Honda Accord', 26000, 5, 'Available for purchase'),
(65, 'Mazda 6', 25000, 4, 'Available for purchase'),
(66, 'Volkswagen Passat', 28000, 3, 'Available for purchase'),
(67, 'Nissan Maxima', 31000, 2, 'Available for purchase'),
(68, 'Hyundai Elantra', 21000, 7, 'Available for purchase'),
(69, 'Kia Optima', 22000, 6, 'Available for purchase'),
(70, 'Subaru Legacy', 27000, 4, 'Available for purchase'),
(71, 'Tesla Model 3', 45000, 5, 'Available for purchase'),
(72, 'BMW 3 Series', 50000, 2, 'Available for purchase'),
(73, 'Audi A6', 55000, 1, 'Available for purchase'),
(74, 'Mercedes E-Class', 60000, 2, 'Available for purchase'),
(75, 'Volvo S60', 48000, 3, 'Available for purchase'),
(76, 'Peugeot 508', 35000, 4, 'Available for purchase'),
(77, 'Renault Talisman', 33000, 3, 'Available for purchase'),
(78, 'Skoda Superb', 34000, 5, 'Available for purchase'),
(79, 'Dodge Charger', 50000, 2, 'Available for purchase'),
(80, 'Chrysler 300', 52000, 1, 'Available for purchase');



INSERT INTO User VALUES 
('AdminJoe', 'John', 'Doe', 'john@email.gr', 'password123', 12345678, 'Favierou', 123),
('AdminJane', 'Jane', 'Smith', 'jane@email.gr', 'securepass', 9876543, 'Gerokostopoulou', 45),
('MechJones', 'Mike', 'Jones', 'mike@email.gr', 'mikepass', 55512345, 'Kolokotroni', 78),
('MechaSarah', 'Sarah', 'Wilson', 'sarah@email.gr', 'sarah123', 11122233, 'Hfaistou', 12),
('david_brown', 'David', 'Brown', 'david@email.gr', 'davidpass', 44455566, 'Astiggos', 34),
('lisa_taylor', 'Lisa', 'Taylor', 'lisa@email.gr', 'lisapass', 77788899, 'Patrews', 56),
('robert_garcia', 'Robert', 'Garcia', 'robert@email.gr', 'robertpass', 22233344, 'Gounari', 89),
('emily_miller', 'Emily', 'Miller', 'emily@email.gr', 'emilypass', 66677788, 'Rhga Fereou', 23),
('thomas_clark', 'Thomas', 'Clark', 'thomas@email.gr', 'thomaspass', 99900011, 'Serrwn', 67),
('amanda_white', 'Amanda', 'White', 'amanda@email.gr', 'amandapass', 33344455, 'Antonys', 90),
('KingVon', 'Ioannhs','Basilopoulos','hitman@email.gr','KingKobra1312',1233124213,'Runaway',45),
('Mparios','Paraskeuas','Ksulokastros','baloket@email.gr','repairs23', 69342243,'Asstigos',69),
('AlbanianKing', 'Aimilianio', 'Corfioti', 'shqipis@email.al', 'QukeZi22', 355455566, 'Favierou', 23),
('Diabito','Xristakios','Mpantrinos','agiotismalamas@email.gr','BedEnjoyer21',69434343,'Amartias', 73),
('Xazas', 'Blashs', 'Restas', 'mamakias@email.gr', 'Apodiksis3', 424455566, 'Kalamata', 32),
('Giannjs', 'John', 'Jonh', 'jamal@email.gr', 'steamerburger2', 333323255, 'Gerou', 90),
('Gatoulhs','Dimitris','Gatopardidis','ioanninaenjoyer@email.gr','Lovers123', 694323433,'Ermou', 34);



INSERT INTO Customer VALUES ('david_brown'),
('lisa_taylor'),
('robert_garcia'),
('emily_miller'),
('thomas_clark'),
('KingVon'),
('Mparios'),
('AlbanianKing'),
('Diabito'),
('Xazas'),
('Giannjs'),
('Gatoulhs');



INSERT INTO Mechanic VALUES ('MechJones'),
('MechaSarah');



INSERT INTO Admin VALUES ('AdminJoe'),
('AdminJane');


INSERT INTO Order_ (OrderID, OrderDate, CustomerName, ProductID, Quantity, Sub_date, F_date, Status_, MechanicName, Price) VALUES
(1, '2023-01-15', 'lisa_taylor', 1, 1, '2023-01-10 10:00:00', '2023-01-20', 'Finished', NULL, 65000),
(2, '2023-02-20', 'robert_garcia', 3, 1, '2023-02-21 10:00:00', '2023-02-25', 'Ready for Pickup', NULL, 42000),
(3, '2023-03-05', 'emily_miller', 6, 1, '2023-03-06 10:00:00', '2023-03-10', 'Pending', NULL, 55000),
(4, '2023-04-10', 'thomas_clark', 8, 1, '2023-04-11 10:00:00', '2023-04-15', 'Cancelled', NULL, 23000),
(5, '2023-05-15', 'thomas_clark', 4, 1, '2023-05-16 10:00:00', '2023-05-20', 'Finished', NULL, 28000),
(6, '2023-06-20', 'emily_miller', 9, 1, '2023-06-21 10:00:00', '2023-06-25', 'Ready for Pickup', NULL, 120000);


INSERT INTO Test_Drive VALUES 
(1, '2023-01-05 10:00:00', 'emily_miller', 'BMW X5', 'Confirmed'),
(2, '2025-02-10 11:30:00', 'lisa_taylor', 'Audi A4', 'Confirmed'),
(3, '2025-03-15 09:15:00', 'thomas_clark', 'Ford Mustang', 'Confirmed'),
(4, '2025-04-20 14:00:00', 'emily_miller', 'Volkswagen Golf', 'Confirmed'),
(5, '2025-05-25 13:45:00', 'robert_garcia', 'Toyota Camry', 'Confirmed'),
(6, '2025-05-26 13:45:00', NULL, 'Toyota Camry', 'Available');


INSERT INTO Repairs VALUES 
('ABC123', 'Toyota Corolla', '2018', 'thomas_clark', 4500),
('XYZ456', 'BMW X5', '2020', 'lisa_taylor', 3000),
('LMN789', 'Ford Focus', '2019', 'emily_miller', 2500),
('DEF012', 'Chevrolet Malibu', '2021', 'thomas_clark', 2000);


INSERT INTO Trade_In (TradeID, CustomerName, TradeDate, CarName, CarModel, Description, Price) VALUES
(1, 'lisa_taylor', '2023-01-10 10:00:00', 'Toyota Corolla', '2018', 'Good condition, minor scratches', NULL),
(2, 'robert_garcia', '2023-01-11 11:30:00', 'Honda Civic', '2019', 'Excellent condition, no issues', NULL),
(3, 'emily_miller', '2023-01-12 14:15:00', 'Ford Focus', '2017', 'Fair condition, needs minor repairs', NULL),
(4, 'robert_garcia', '2023-01-13 09:45:00', 'Chevrolet Malibu', '2020', 'Like new, low mileage', NULL),
(5, 'thomas_clark', '2023-01-14 16:20:00', 'Nissan Altima', '2016', 'Good condition, well-maintained', NULL),
(6, 'thomas_clark', '2023-01-15 13:05:00', 'Subaru Impreza', '2018', 'Very good condition, all-wheel drive', NULL);



INSERT INTO CarCustomer (CarCustomerID, CarPlate, CustomerName) VALUES
(1, 'ABC123', 'thomas_clark'),
(2, 'XYZ456', 'lisa_taylor'),
(3, 'LMN789', 'emily_miller'),
(4, 'DEF012', 'thomas_clark');


INSERT INTO ServiceHistory (ServiceID, CarCostumerID, ServiceType, ServiceDate, Description) VALUES
(1, 1, 'Oil Change', '2023-01-15', 'Regular maintenance'),
(2, 2, 'Brake Replacement', '2023-02-20', 'New brake pads and rotors'),
(3, 3, 'Tire Rotation', '2023-03-10', 'Rotated tires for even wear'),
(4, 4, 'Battery Replacement', '2023-04-05', 'Replaced old battery with new one');



INSERT INTO Leasing (LeasingID, CustomerName, ProductID, Start_date, Final_date, Status_, Price) VALUES
(1, 'robert_garcia', 2, '2023-01-01', '2023-12-31', 'Finished', 4500),
(2, 'lisa_taylor', 7, '2023-02-01', '2023-11-30', 'Renting', 4900),
(3, 'emily_miller', 3, '2023-03-01', '2024-02-29', 'Renting', 5200),
(4, 'thomas_clark', 5, '2023-04-01', '2024-04-02', 'Cancelled', 4800),
(5, NULL, 7,2025-05-01, 2023-05-11, 'Available', 4800);


INSERT INTO Messages (MessageID, MessageText, SenderName) VALUES
(1, 'Do we have any electric vehicles in stock?', 'MechJones'),
(2, 'I am stack help me!', 'MechJones'),
(3, 'Get a load of this guy!:)','MechaSarah'),
(4, 'Meeting in 10 minutes', 'AdminJoe'),
(5, 'I need a coffee break!', 'AdminJane'),
(6, 'Let\'s review the project status', 'MechJones'),
(7, 'Can you send me the latest report?', 'MechaSarah'),
(8, 'I will be out of office tomorrow', 'MechJones'),
(9, 'Don\'t forget the team lunch!', 'AdminJane'),
(10, 'We need to finalize the budget', 'AdminJoe');




INSERT INTO Trade_in_eleuthera (TradeINID, FreeDates, Status_) VALUES
(1, '2025-06-01 09:00:00', 'Available'),
(2, '2025-06-02 09:00:00', 'Unavailable'),
(3, '2025-06-01 09:30:00', 'Available'),
(4, '2025-06-01 10:00:00', 'Available'),
(5, '2025-06-01 10:30:00', 'Available'),
(6, '2025-06-01 11:00:00', 'Available'),
(7, '2025-06-01 11:30:00', 'Available'),
(8, '2025-06-01 12:00:00', 'Available'),
(9, '2025-06-01 12:30:00', 'Available'),
(10, '2025-06-01 13:00:00', 'Available'),
(11, '2025-06-01 13:30:00', 'Available'),
(12, '2025-06-01 14:00:00', 'Available'),
(13, '2025-06-02 9:30:00', 'Available'),
(14, '2025-06-02 10:00:00', 'Available'),
(15, '2025-06-02 10:30:00', 'Available'),
(16, '2025-06-02 11:00:00', 'Available'),
(17, '2025-06-02 11:30:00', 'Available'),
(18, '2025-06-02 12:00:00', 'Available'),
(19, '2025-06-02 12:30:00', 'Available'),
(20, '2025-06-02 13:00:00', 'Available'),
(21, '2025-06-02 13:30:00', 'Available'),
(22,'2025-06-02 14:00:00', 'Available'),
(23,'2025-06-03 9:00:00', 'Unavailable'),
(24,'2025-06-03 9:30:00','Unavailable'),
(25,'2025-06-03 10:00:00','Unavailable'),
(26,'2025-06-03 10:30:00','Unavailable'),
(27,'2025-06-03 11:00:00','Unavailable'),
(28,'2025-06-03 11:30:00','Unavailable'),
(29,'2025-06-03 12:00:00','Unavailable'),
(30,'2025-06-03 12:30:00','Unavailable'),
(31,'2025-06-03 13:00:00','Unavailable'),
(32,'2025-06-03 13:30:00','Unavailable'),
(33,'2025-06-03 14:00:00','Unavailable'),
(34,'2025-06-04 9:00:00','Unavailable'),
(35,'2025-06-04 9:30:00','Unavailable'),
(36,'2025-06-04 10:00:00','Unavailable'),
(37,'2025-06-04 10:30:00','Unavailable'),
(38,'2025-06-04 11:00:00','Unavailable'),
(39,'2025-06-04 11:30:00','Unavailable'),
(40,'2025-06-04 12:00:00','Unavailable'),
(41,'2025-06-04 12:30:00','Unavailable'),
(42,'2025-06-04 13:00:00','Unavailable'),
(43,'2025-06-04 13:30:00','Unavailable'),
(44,'2025-06-04 14:00:00','Unavailable');


INSERT INTO Package (PackageID, PackageOrderID, PackcageStatus) VALUES
(1, 1, 'Packed'),
(2, 2, 'Packed'),
(3, 3, 'Unpacked');


INSERT INTO Wishlist (WishlistID, CustomerName, ProductID,NewsLetter) VALUES
(1, 'lisa_taylor', 9, TRUE);